...
def extract_claims_for_section(
    ...
    variants = [source_slice]
    if len(source_slice) > 9000:
        variants.append(source_slice[:9000])
    for variant in variants:
        payload = {
            "heading": section.heading,
            "source_slice": variant,
        ...
        for attempt in range(2):
            try:
                ...
                if attempt == 1:
                    log.warning(
                        "Failed to parse claim extraction JSON for section=%s",
                        section.heading,
                    )
...
def _validate_claims(raw: dict, section: SectionMeta) -> list[CoreClaim]:
    ...
    for idx, item in enumerate(raw.get("core_claims", []), start=1):
        ...
        evidence = Evidence(
            ...
            source_lines=[
                int(x)
                for x in evidence_data.get("source_lines", [])
                if str(x).isdigit()
            ],
        )
        claims.append(
            CoreClaim(
                ...
                source_lines=[
                    int(x) for x in item.get("source_lines", []) if str(x).isdigit()
                ],
