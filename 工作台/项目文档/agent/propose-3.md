...
本方案是 Entropy Zero AI Agent 的**最终重构方案**，**完全替代** `原propose-3.md`。
本文档为单一信息源（self-contained），删除 原propose-3 后仍可凭此文档独立完成 Agent 的重构与实施。

历史脉络（仅供背景，不影响实施）：

- **propose-1/2**：早期多步 LLM 方案，已废弃
- **原propose-3**：将 13 次 LLM 调用收敛为 1 次，但仍存在三个未解问题——
...
-- flashcards: 卡片元数据（沿用 原propose-3 的字段定义，归并到本次 migration）
...
└── specs/
    ...
    └── bilingual-llm-routing/spec.md              （沿用 原propose-3 的 spec，补充 extract prompt）
```
