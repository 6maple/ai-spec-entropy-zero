...
本变更合并的代码路径 MUST NOT 包含向大模型供应商发起的 HTTP 客户端调用；Phase 1 处理器 MUST 为规则或确定性占位实现。错误路径 SHALL 使用 `error_code`、`error_message`（及可选 `debug_hint`），不得用未定义结构掩盖失败。

#### Scenario: Reject processor implementation with vendor client

- **WHEN** 代码审查发现针对外部 LLM API 的 HTTP 客户端被用于本管线
- **THEN** 该实现 MUST 被拒绝，直至改为占位或内部规则实现
